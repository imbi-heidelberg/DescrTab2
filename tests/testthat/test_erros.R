...
test_that("warnings when lists which are supposed to be named dont have names are produced",
          {
            ...
            expect_warning(expect_warning(descr(iris, summary_stats_cat = list(mean))))
...
test_that("error is produces when summary_stats lists are misspecified",
          {
            ...
            expect_warning(expect_error(descr(iris, var_options = list(a=list(summary_stats=list("mean"))))))
...
test_that("error is produced when format_p in var_options is misspecified",
          {
            expect_error(descr(iris, var_options = list(Sepal.Width = list(
              format_p = list("mean")
            ))))
          })
...
test_that("test override is check",
          {
            expect_warning(descr(iris, var_options = list(Sepal.Length = list(test_override =
                                                                                "abc"))))
          })
...
test_that("forgotten indices in paired tests lead to errors",
          {
            expect_message(expect_message(descr(dat %>% select(-cat), "group", test_options = list(paired=TRUE))))
            expect_message(expect_message(descr(dat %>% select(-extra), "group", test_options = list(paired=TRUE)) ))
          }
          )
