...
test_that("forgotten indices in paired tests lead to errors",
          {
            expect_message(descr(dat %>% select(-cat), "group", test_options = list(paired=TRUE)) %>% print(silent=TRUE))
            expect_message(descr(dat %>% select(-extra), "group", test_options = list(paired=TRUE)) %>% print(silent=TRUE))
          }
          )
