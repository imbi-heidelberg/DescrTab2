...
# Introduction
<font size=4> In this document, we compare various DescrTab2 tests with their SAS equivalents.
This document is created by including in the .html SAS output. Unfortunately, this has ugly side effects for the formatting of this document, but everything should still be readable. </font>
...
descr(dat_wilcox.test_1_sample, test_options = c(nonparametric=T))
```
...
descr(dat_wilcox.test_2_sample, "group", test_options = c(nonparametric=T))
```
...
descr(dat, "group", test_options = list(nonparametric=T, indices=idx, paired=T))
...
descr(dat[, c("Sex", "distance")], "Sex", test_options = list(paired=T, indices=dat$Subject))
...
descr(dat, "gender", test_options = c(exact=T))
...
Exact::exact.test(table(dat), method="boschloo", to.plot = F)
```
