...
`DescrTab` is a package to create descriptive statistics tables. It supports a variety of different customization options and can be used in .Rmd files in conjunction with knitr.
...
Note the chunk option ` results='asis' `. The `des.print` function returns \LaTeX code by default. The directive `results='asis'` tells knitr to interpret the output of des.print as if it were pasted into the .tex file that is used to create the pdf output. Without the `results='asis'` chunk option, the table will not be rendered properly in the output document.

### The most common usecase

Usually a study population is divided into serveral groups of interest, and we would like to examine a comparison of these. This can be achieved by using the `group` parameter:
...
You can assign labels to the different variables using the ` var.names ` parameter. Additionally, setting ` data.names=F ` will surpress output of the variable names and only print out the given labels.
...
Firstly, you can try to decrease the fontsize via the ` fsize ` parameter (default is ` fsize = 11 `), which will shrink the whole table:
...
You could also try temporarily switching your document to landscape mode. This can by done by including the following in your preamble:
